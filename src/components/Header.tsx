      ...
      {session?.user ? (
        <div className="flex items-center gap-2">
          <div className="w-8 h-8 rounded-full">
            <Image src={session?.user?.image as string} alt="userIamge" width={200} height={200} className="w-full h-full rounded-full object-cover" />
          </div>
          <SignOut/>
        </div>
      ) : (
        <Link href={"/signin"} className="text-sm font-semibold hover:text-white duration-300">
          Sign in
        </Link>
      )}
    </div>
